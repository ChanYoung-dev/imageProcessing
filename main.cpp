...
void get_hist(uchar** img,uchar** result_img, int X_Size, int Y_Size)
{
   int i, j, tmp;
   double tmp1;
   int t, tp, range, hrange;
   ...
   for (i = 0; i < Y_Size; ++i)
      for (j = 0; j < X_Size; ++j)
         result_img[i][j] = histogramEqual[img[i][j]];   //역함수를 이용해 평활화 영상결과를 img에 넣음
}
...
int main(int argc, char* argv[])
{
   ...
   read_unmatrix(imgSize.width, imgSize.height, img, argv[1]); // read Image
   ...
   get_hist(img, result_img,imgSize.width, imgSize.height);
